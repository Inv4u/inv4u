import { Check } from 'lucide-react';
import PhoneMockup from './PhoneMockup';
import { site, telHref, whatsappHref } from '@/lib/site';

const trustSignals = [
  'בלי אפליקציה לאורחים',
  'שיחות AI בעברית',
  'תמיכה ופיתוח בישראל',
];

export default function HeroSection() {
  return (
    <section className="bg-white px-5 pb-16 pt-10 md:pb-24 md:pt-20" dir="rtl">
      <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-2 lg:gap-16">
        {/* text column */}
        <div className="space-y-6 text-center lg:text-right">
          <h1 className="text-4xl font-extrabold leading-tight text-brand-navy md:text-5xl">
            הטכנולוגיה שמנהלת את האירוע שלך
          </h1>

          <p className="mx-auto max-w-md text-lg leading-relaxed text-gray-500 lg:mx-0">
            הזמנות בוואטסאפ, אישורי הגעה בשיחות AI, וניהול אורחים — במקום אחד.
          </p>

          <div className="flex flex-col items-center gap-3 pt-1 sm:flex-row sm:justify-center lg:justify-start">
            <a
              href={whatsappHref}
              target="_blank"
              rel="noopener noreferrer"
              className="btn-primary w-full sm:w-auto"
            >
              <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current" aria-hidden="true">
                <path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 018.413 3.488 11.824 11.824 0 013.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.523 5.27l-.999 3.648 3.965-1.039zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" />
              </svg>
              דברו איתנו בוואטסאפ
            </a>
            <a
              href={telHref}
              dir="ltr"
              className="text-sm font-semibold text-gray-500 underline-offset-4 transition hover:text-brand-navy hover:underline"
            >
              או חייגו {site.phoneDisplay}
            </a>
          </div>

          <ul className="flex flex-wrap justify-center gap-x-5 gap-y-2 pt-2 lg:justify-start">
            {trustSignals.map((label) => (
              <li key={label} className="flex items-center gap-1.5 text-sm text-gray-600">
                <Check className="h-4 w-4 flex-shrink-0 text-brand-blue" strokeWidth={2.5} />
                {label}
              </li>
            ))}
          </ul>
        </div>

        {/* phone column */}
        <div className="flex justify-center lg:justify-start">
          <PhoneMockup />
        </div>
      </div>
    </section>
  );
}
